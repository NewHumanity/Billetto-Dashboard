...
const AttendeesTable: React.FC<AttendeesTableProps> = ({ attendees, currency, requestSort, sortConfig, onSelectAttendee, currentPage, itemsPerPage }) => {
  ...
  const statusColorMap: { [key: string]: string } = {
    sold: 'bg-green-500/20 text-green-500 dark:text-green-400',
    refunded: 'bg-yellow-500/20 text-yellow-500 dark:text-yellow-400',
    cancelled: 'bg-red-500/20 text-red-500 dark:text-red-400',
    reserved: 'bg-blue-500/20 text-blue-500 dark:text-blue-400',
    default: 'bg-slate-500/20 text-slate-500 dark:text-slate-400'
  };
  
  const SortableHeader: React.FC<{ title: string, sortKey: keyof Attendee}> = ({ title, sortKey }) => {
    const isSorted = sortConfig?.key === sortKey;
    return (
        <th scope="col" className="py-3.5 px-4 text-left text-sm font-semibold text-slate-800 dark:text-white">
        ...
        <tbody className="divide-y md:divide-y-0 divide-gray-200 dark:divide-slate-700 bg-white dark:bg-slate-800/50">
          {attendees.map((attendee, index) => (
            ...
            >
              ...
              <td data-label="Price" className="whitespace-nowrap py-4 px-4 text-sm text-slate-600 dark:text-slate-300">
