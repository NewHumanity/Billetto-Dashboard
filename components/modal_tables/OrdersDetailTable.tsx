...
import { Order, LedgerEntry } from '../../types';
...
// --- NEW COMPONENT ---
interface GrossRevenueDetailTableProps {
  orders: Order[];
  ledgerEntries: LedgerEntry[];
  pushView: (view: any) => void;
}

export const GrossRevenueDetailTable: React.FC<GrossRevenueDetailTableProps> = ({ orders, ledgerEntries, pushView }) => {
    
    const ordersWithGrossRevenue = React.useMemo(() => {
        const grossRevenueByOrder = new Map<string, number>();
        ledgerEntries.forEach(entry => {
            if (entry.order_id && (entry.entry_type === 'ORDER_REVENUE' || entry.entry_type === 'DISCOUNTS')) {
                const orderId = String(entry.order_id);
                const current = grossRevenueByOrder.get(orderId) || 0;
                grossRevenueByOrder.set(orderId, current + entry.amount);
            }
        });
        ...
        for (const [orderId, grossRevenue] of grossRevenueByOrder.entries()) {
            ...
            if (order) {
                result.push({ ...order, grossRevenue });
            }
        }
    ...
    if (ordersWithGrossRevenue.length === 0) {
        return <p className="text-slate-500 dark:text-slate-400 text-center py-8">No gross revenue data available for this view.</p>;
    }
        ...
        <div className="overflow-x-auto">
            <table className="min-w-full responsive-table">
                ...
                <tr>
                    <th className="py-3 px-4 text-left text-xs font-semibold text-slate-700 dark:text-white uppercase tracking-wider">Date</th>
                    <th className="py-3 px-4 text-left text-xs font-semibold text-slate-700 dark:text-white uppercase tracking-wider">Buyer</th>
                    <th className="py-3 px-4 text-right text-xs font-semibold text-slate-700 dark:text-white uppercase tracking-wider">Gross Revenue</th>
                </tr>
                </thead>
                <tbody className="divide-y md:divide-y-0 divide-gray-200 dark:divide-slate-700">
                {ordersWithGrossRevenue.map((order) => (
                    <tr 
                        key={order.id}
                        onClick={() => handleRowClick(order)}
                        className="cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors"
                    ...
                    <td data-label="Date" className="whitespace-nowrap py-3 px-4 text-sm text-slate-600 dark:text-slate-300">{formatDate(order.created_at)}</td>
                    <td data-label="Buyer" className="py-3 px-4 text-sm font-medium text-slate-900 dark:text-white">{order.buyer_name}</td>
                    <td data-label="Gross Revenue" className="whitespace-nowrap py-3 px-4 text-sm font-semibold text-right text-slate-900 dark:text-white">
                        {formatCurrency(order.grossRevenue, order.currency)}
                    </td>
                    </tr>
                ))}
                </tbody>
