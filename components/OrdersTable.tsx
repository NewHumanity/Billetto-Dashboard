    ...
    <div className="overflow-x-auto">
      <table className="min-w-full responsive-table">
        <thead className="bg-slate-900/80 sticky top-0">
          <tr>
            <th scope="col" className="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-white">#</th>
        ...
        <tbody className="divide-y md:divide-y-0 divide-slate-700 bg-slate-800/50">
          {orders.map((order, index) => (
            ...
            >
              <td data-label="#" className="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-slate-400">
              ...
              <td data-label="Event" className="whitespace-nowrap py-4 px-4 text-sm text-slate-300 truncate max-w-xs">{(order.event && typeof order.event === 'object') ? order.event.name : 'N/A'}</td>
